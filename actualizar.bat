...
echo  [1/7] Obteniendo version de hoy...
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyy.MM.dd"') do set VER=%%i
set TAG=v%VER%
echo        %TAG%
echo.

echo  [2/7] Limpiando cache anterior...
...
echo  [6/7] Instalando localmente...
...
echo  [7/7] Publicando en GitHub (inicia build automatico para Windows y Mac)...
git add version.txt
git commit -m "Version %VER%"
git push origin master
if errorlevel 1 ( echo  ERROR: No se pudo hacer push a master. & pause & exit /b 1 )
git tag -f "%TAG%"
